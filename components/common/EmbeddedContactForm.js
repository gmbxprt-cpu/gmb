...
export default function EmbeddedContactForm() {
  ...
  // reCAPTCHA site key (client-side) — set in .env.local or Vercel as NEXT_PUBLIC_RECAPTCHA_SITE_KEY
  ...
  const handleSubmit = async (e) => {
    ...
    const formData = {
      ...
      token: captchaValue, // send token to backend for verification
    };
    ...
    try {
      ...
      if (response.ok) {
        // reset widget and captcha state
  ...
  return (
    <div className="bg-gradient-to-br from-white to-slate-50 p-6 sm:p-8 rounded-lg shadow-2xl border border-slate-200/80">
      <h3 className="text-2xl font-bold text-black text-center mb-1">
        Get a <span className="text-blue-600">Free Audit</span>
      </h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-center">
          {siteKey ? (
            ...
            <p className="text-sm text-yellow-600">reCAPTCHA site key not configured (local dev?). Add NEXT_PUBLIC_RECAPTCHA_SITE_KEY to env.</p>
          )}
        </div>
