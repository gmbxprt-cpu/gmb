...
const OnetimeCard = ({ service, isFeatured = false }) => {
    ...
    return (
        <div className={`relative bg-white rounded-2xl p-8 flex flex-col shadow-lg transition-all duration-300 ${isFeatured ? 'border-2 border-blue-600 transform scale-105' : 'border border-slate-200'}`}>
            {isFeatured && ( <span className="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider">Recommended</span> )} {/* Changed 'Most Popular' to 'Recommended' if you wish */}
