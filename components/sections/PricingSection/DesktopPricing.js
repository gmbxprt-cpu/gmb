...
/* ---------- Country detection helper (client-side, with caching) ---------- */
async function fetchCountryEdgeFirst() {
  // Try /api/geo (edge) -> ipwhois.app -> ipapi.co
...
const PricingCard = ({ plan, numericPrice, currency, isFeatured = false, isYearly, isIndia }) => {
  ...
  return (
    <div className={`relative bg-white rounded-2xl p-8 flex flex-col shadow-lg transition-all duration-300 ${borderStyle}`}>
        ...
        <ul className="mt-8 space-y-4 text-left">
          {allPricingFeatures.map(feature => {
            ...
            return (
              <li key={feature.key} className="flex items-center text-slate-800 font-medium">
                <CheckIcon />
                <span className="ml-3">{feature.text.replace(/Keywords|Months/g, '').trim()}: <span className="font-bold text-blue-600">{featureValue}</span></span>
              </li>
            );
          })}
        </ul>
      ...
      {/* Call now (India) or WhatsApp (others). No prefilled message */}
      {isIndia ? (
        <a
          href="tel:+917009364216"
          className="flex items-center justify-center w-full mt-8 py-3 px-6 text-center rounded-lg font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-colors duration-300"
          aria-label="Call Now"
        >
          <PhoneIcon /> Call Now
        </a>
        ...
        <a
          ...
          className="flex items-center justify-center w-full mt-8 py-3 px-6 text-center rounded-lg font-semibold text-white bg-green-500 hover:bg-green-600 transition-colors duration-300"
...
export default function DesktopPricing() {
  ...
  const [country, setCountry] = useState(null); // country code (e.g., 'IN' or 'US')
  ...
  // USD fixed rates for other countries (monthly)
  const usdFixed = { Silver: 149, Gold: 249, Platinum: 399 };

  const computeNumericPrice = (plan) => {
    if (currency === "INR") {
      // assume plan.price already has Indian monthly numeric value in plansData
      const base = Number(plan.price ?? 0);
      return isYearly ? Math.round(base * 0.50) : base;
    } else {
      const baseUsd = Number(usdFixed[plan.name] ?? 0);
      return isYearly ? Math.round(baseUsd * 0.50) : baseUsd;
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 text-center">
        <Fade direction="up" cascade damping={0.1} triggerOnce>
          ...
          <div className="mt-8 flex justify-center items-center space-x-4">
            ...
            <span className={`font-semibold ${isYearly ? "text-blue-600" : "text-gray-500"}`}> Yearly <span className="text-green-500 text-sm">(Save 50%)</span> </span>
          </div>

          <div className="mt-16 grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch relative">
            <PricingCard plan={plansData.silver} numericPrice={computeNumericPrice(plansData.silver)} currency={currency} isYearly={isYearly} isIndia={isIndia} />
            <PricingCard plan={plansData.gold} numericPrice={computeNumericPrice(plansData.gold)} currency={currency} isFeatured={true} isYearly={isYearly} isIndia={isIndia} />
            <PricingCard plan={plansData.platinum} numericPrice={computeNumericPrice(plansData.platinum)} currency={currency} isYearly={isYearly} isIndia={isIndia} />
          </div>
        </Fade>
      </div>
    </section>
